"""Export Phase 45 Order Type / TIF / Venue Semantics candidates for strict audit."""
...
PACKAGE_GATE = resolve_repo_path("docs", "reports", "phase45_order_semantics_candidate_audit_package_quality_gate.json", start_file=__file__)
...
def load_candidates() -> list[dict[str, Any]]:
    ...
    for path in sorted(cand_dir.glob("cand_20260612_phase45_order_semantics_*.json")):
        data = read_json(path)
...
def package_quality_gate(candidates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(candidates) != 6:
        failures.append(f"expected 6 candidate_ready Order Semantics candidates, got {len(candidates)}")
    if actual != expected:
    ...
    return {
        "gate_id": "phase45_order_semantics_candidate_audit_package_quality_gate",
        "checked_at": TODAY,
        "phase": PHASE,
        "task_id": TASK_ID,
        ...
        "warnings": [
            ...
            "Order Semantics 只能用于订单类型、TIF、venue adapter、STP/SMP、maker/taker fee 和执行证据边界，不得输出交易动作或路由建议。",
            "FIX、CME、Nasdaq、Coinbase、Binance、Kraken、SEC、CFA 来源具有协议、venue、产品、账户模式、API version 和市场语境边界。",
        ],
    }
...
def build_package(candidates: list[dict[str, Any]], gate: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "scope": {
            ...
            "partition": PARTITION,
            "batch": "P45-F Order Type / TIF / Venue Semantics / 订单类型、TIF 与交易所语义",
            "candidate_count": len(candidates),
            "target": "审计 Order Type / TIF / Venue Semantics 候选，包括订单类型语义、TIF、post-only/reduce-only、self-trade prevention、exchange-specific order type caveat 和 maker/taker fee 边界。",
        },
        "hard_boundaries": {
            ...
            "must_not_generate": ["买卖点", "仓位", "杠杆", "止损止盈参数", "实盘执行建议", "路由建议", "费用套利", "订单提交许可", "自动撤单/改单"],
        },
        "audit_instructions": [
            "必须搜索相关的专业网站、官方文档、监管资料、交易所/协议资料、broker/venue API 文档、案例和数据，对本审计包进行严格审计。",
            "逐条检查来源是否足以支撑 claim；FIX、CME、Nasdaq、Coinbase、Binance、Kraken、SEC、CFA 不得被过度泛化。",
            "检查 Order Semantics 是否正确归入 Live Execution / Order Semantics，并保持与 Replay/Simulation order semantics、TCA、Audit Trail、Layered Risk 的 owner 边界。",
            "检查每条候选是否清楚声明适用范围、不适用场景、假设、限制、来源质量和 AI 使用边界。",
            "重点审计 order type、TIF、post-only、reduce-only、STP/SMP、venue-specific order behavior、maker/taker fee 和 order-event evidence。",
            "检查是否有中文乱码、mock/test 污染、项目私有策略参数、账户事实、密钥、真实订单、真实费率、真实阈值或实盘敏感信息。",
            ...
            "candidate_results": [
                {
                    ...
                    "classification_assessment": {
                        ...
                        "expected_branch": "Trading Engineering / Live Execution / Order Semantics",
