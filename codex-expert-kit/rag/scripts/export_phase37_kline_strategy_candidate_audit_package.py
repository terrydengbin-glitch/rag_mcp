"""Export Phase 37 Kline / Strategy Engineering candidate audit package."""
...
AUDIT_PACKAGE = resolve_repo_path(
    "docs", "audit", "phase37_kline_strategy_candidate_audit_package_20260611.json", start_file=__file__
)
QUALITY_GATE = resolve_repo_path(
    "docs", "reports", "phase37_kline_strategy_candidate_audit_package_quality_gate.json", start_file=__file__
)
...
def load_candidates() -> list[dict[str, Any]]:
    candidates = []
    for path in sorted(CAND_DIR.glob("cand_20260611_phase37_kline_strategy_*.json")):
        data = read_json(path)
...
def main() -> None:
    ...
    package = {
        ...
        "package_id": "phase37_kline_strategy_candidate_audit_package_20260611",
        "phase": PHASE,
        "task_id": TASK_ID,
        "created_at": TODAY,
        "purpose": "外部 AI/人工对 Phase 37 Kline / Strategy Engineering 12 条候选进行严格审计，判断每条只能进入 accepted_for_draft、needs_more_evidence 或 rejected。",
        "hard_boundaries": {
        ...
        "audit_instructions": [
            "必须搜索相关专业网站、论文、官方文档、资料、案例和数据，对候选来源与 claim 进行严格审计。",
            "检查每条 claim 是否被来源充分支持，是否夸大技术指标、K线形态、趋势结构或多周期信号的适用范围。",
        ...
        "scope_notes": [
            "本批归入 KB_02_KLINE_STRATEGY，因为当前 knowledge_tree.md 与 metadata_schema.md 使用该运行时分区。",
            "历史范围文档中的 KB_03_STRATEGY_ENGINEERING 是旧命名，审计时应以当前运行时树和 schema 为准。",
            "本批候选只做 K线/策略工程方法边界，不证明任何策略盈利。",
        ],
