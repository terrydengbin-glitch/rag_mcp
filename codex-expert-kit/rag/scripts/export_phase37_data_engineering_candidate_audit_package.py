"""Export Phase 37 Data Engineering candidates for strict external audit."""
...
PACKAGE_ID = "phase37_data_engineering_candidate_audit_package_20260611"
...
PACKAGE_GATE = resolve_repo_path(
    "docs", "reports", "phase37_data_engineering_candidate_audit_package_quality_gate.json", start_file=__file__
)
...
def load_candidates() -> list[dict[str, Any]]:
    candidates = []
    for path in sorted(CAND_DIR.glob("cand_20260611_phase37_data_engineering_*.json")):
...
def package_quality_gate(candidates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    expected_tasks = {f"P37-B-D{idx:02d}" for idx in range(1, 13)}
    ...
    return {
        "gate_id": "phase37_data_engineering_candidate_audit_package_quality_gate",
        "checked_at": TODAY,
...
def build_package(candidates: list[dict[str, Any]], gate: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "scope": {
            ...
            "batch": "P37-B Data Engineering",
            "candidate_count": len(candidates),
            "target": "审计交易数据工程本体知识候选，包括时间戳、时区、缺失重复、OHLCV、特征时间、数据版本、symbology、复权/换月、异常值和数据质量报告。",
        },
        "hard_boundaries": {
        ...
        "audit_instructions": [
            "必须搜索相关的专业网站、资料、案例和数据，对本审计包进行严格审计。",
            "逐条检查来源是否足以支撑 claim，vendor/framework 文档不能被过度泛化。",
            "检查是否正确归类到 Trading Engineering / Data Engineering，不得混入 AI Engineering 训练、RAG、MCP 或 LLMOps 本体。",
            "检查是否清楚声明适用范围、不适用场景、假设、限制和 AI 使用边界。",
            "检查 timestamp/timezone/schema/version/corporate action 等规则是否存在过强、过宽或缺源问题。",
...
def main() -> None:
    ...
    if gate["gate_status"] != "pass":
        raise SystemExit(f"package quality gate failed: {gate['failures']}")
    print(
        json.dumps(
            {
                "candidate_count": len(candidates),
                "audit_package": str(AUDIT_PACKAGE),
                "quality_gate": str(PACKAGE_GATE),
