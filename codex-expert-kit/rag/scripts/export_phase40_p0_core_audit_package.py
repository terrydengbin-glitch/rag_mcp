"""Export Phase 40 P0-Core candidate AI audit package.
...
AUDIT = resolve_repo_path("docs", "audit", "phase40_candidate_audit_package_20260610.json", start_file=__file__)
QUALITY = resolve_repo_path("docs", "reports", "phase40_candidate_quality_gate.json", start_file=__file__)


EXPECTED_P0_CORE = {f"P40-C{i:02d}" for i in range(1, 19)}
...
def load_candidates() -> list[dict[str, Any]]:
    ...
    for path in sorted(CAND_DIR.glob("cand_20260610_phase40_*.json")):
        ...
        if str(raw.get("research_task_id", "")).startswith("P40-C"):
...
def quality_gate(candidates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in candidates:
        ...
        if research_task_id not in EXPECTED_P0_CORE:
    ...
    for missing in sorted(EXPECTED_P0_CORE - seen):
        failures.append({"research_task_id": missing, "failure": "missing_candidate"})
    return {
        "report_id": "phase40_candidate_quality_gate",
        "generated_at": TODAY,
        "scope": "Phase 40 P0-Core candidate audit package",
...
def main() -> int:
    ...
    package = {
        "package_id": "phase40_candidate_audit_package_20260610",
        "package_type": "candidate_ai_audit_package",
        "generated_at": TODAY,
        "phase": "40",
        "title": "Phase 40 AI Continuous Learning 与再训练闭环 P0-Core 候选知识审计包",
        "purpose": "统一审计 Phase 40 P0-Core 持续学习候选知识，确认来源充分性、适用边界、冲突风险、AI 使用安全和跨分支路由。",
        "hard_boundaries": [
