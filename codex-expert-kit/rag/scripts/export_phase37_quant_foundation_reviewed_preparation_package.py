"""Export Phase 37 Quant Foundation reviewed/caveat_only audit package.

The Phase 37 Quant Foundation candidates have passed strict audit only as
``accepted_for_draft``. This script prepares the next external audit package
that asks whether each candidate can be promoted to formal
...
TASK_ID = "CEK-TA-379"
PACKAGE_ID = "phase37_quant_foundation_reviewed_preparation_audit_package_20260611"
...
REPORT_PATH = resolve_repo_path(
    "docs", "reports", "phase37_quant_foundation_reviewed_preparation_gap_report.json", start_file=__file__
)
...
def is_scope_candidate(candidate: dict[str, Any]) -> bool:
    return candidate_id(candidate).startswith("cand_20260611_phase37_")
...
def package_candidate(candidate: dict[str, Any], gaps: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "review": {
            "confidence": deep_get(candidate, ("review", "confidence")),
...
def build_package(candidates: list[dict[str, Any]], gate: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "scope": {
            ...
            "input_condition": "accepted_for_draft candidates only; reviewed_allowed must currently be false.",
        },
        "audit_instruction": {
            "language": "zh-CN",
            "primary_goal": "严格判断这些 Quant Foundation 候选是否可以从 accepted_for_draft 进入 formal reviewed/caveat_only。",
            "must_search": "必须搜索相关的专业网站、资料、案例和数据，对审计报告进行严格审计；不能只依赖候选包内摘要。",
            "must_check": [
                "来源是否足以支撑 claim，尤其是定义、边界、成本、样本量、风险和执行约束。",
                "是否存在把交易教育材料、供应商文章或单一来源过度上升为通用规则的问题。",
                "是否存在和现有 CEK-TA formal knowledge 的可见冲突或重复。",
                "是否清楚区分 Trading Engineering 规则本体与 AI Engineering 引用/训练/检索规则。",
                "是否包含买卖点、仓位、杠杆、止损止盈、实盘执行或投资建议风险。",
                "是否需要补充页码级书籍证据、论文、监管/交易所资料或机构级文档。",
            ],
            "hard_boundaries": [
                "candidate 不是正式知识。",
...
def main() -> int:
    ...
    gate = {
        "pass": bool(candidates) and not any(packaged_gaps.values()),
    ...
    report = {
        "report_id": "phase37_quant_foundation_reviewed_preparation_gap_report",
        "generated_at": TODAY,
        "task_id": TASK_ID,
        "package_id": PACKAGE_ID,
        ...
        "quality_gate": gate,
        "next_action": "将审计包交给外部 AI/人工严格审计；只有返回 reviewed_allowed=true 的候选才能进入 CEK-TA-196 formal reviewed/caveat_only 转换。",
    }
