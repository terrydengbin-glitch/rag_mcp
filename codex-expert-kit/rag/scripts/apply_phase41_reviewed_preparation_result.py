"""Apply Phase 41 reviewed-preparation audit result.

The reviewed-preparation audit may authorize accepted_for_draft candidates to
become formal reviewed knowledge. This script converts only decisions with
reviewed_allowed=true, marks remaining items as needs_more_evidence, and keeps
approved/default-guidance/hard-gate disabled.
"""
...
SOURCE_PACKAGE_ID = "phase41_ai_passed_reviewed_preparation_audit_package_20260610"
EXPECTED_COUNT = 22
...
REPORT_PATH = resolve_repo_path("docs", "reports", "phase41_reviewed_preparation_import_report.json", start_file=__file__)
...
def shape_conflict_audit(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "resolution_summary": raw.get(
            "resolution_summary",
            "reviewed-preparation 审计确认没有 Trading Engineering 本体误路由；reviewed 不等于 approved。",
        ),
...
def build_content(candidate: dict[str, Any], decision: dict[str, Any]) -> dict[str, Any]:
    ...
    required_followups = dedupe(
        as_list(deep_get(candidate, ("review", "open_questions"), []))
...
def build_machine_gate() -> dict[str, Any]:
    return {
        ...
        "reason": "Phase 41 reviewed-preparation 审计允许转 formal reviewed；可审计检索，但尚未人工 approved，不能默认指导或 hard gate。",
...
def candidate_to_knowledge(candidate: dict[str, Any], decision: dict[str, Any]) -> dict[str, Any]:
    ...
    decision_log.append(
        {
            ...
            "decision": "reviewed",
            "reason": f"{TASK_ID}: reviewed-preparation 审计 accepted_for_draft 且 reviewed_allowed=true；写入 formal reviewed，仍非 approved。",
        }
    )
    ...
    return {
        ...
        "metadata": {
            ...
            "classification_notes": "Phase 41 formal reviewed knowledge；reviewed-preparation 审计允许 reviewed，但不是 approved/default guidance。",
...
def validate_audit_result(audit_result: dict[str, Any]) -> list[str]:
    ...
    for decision in decisions:
        ...
        if decision.get("decision") not in {"accepted_for_draft", "needs_more_evidence", "rejected"}:
            errors.append(f"{decision.get('candidate_id')}: unsupported decision")
    return errors
...
def update_candidate_backlink(candidate: dict[str, Any], item: dict[str, Any], knowledge_path: Path, decision: dict[str, Any]) -> None:
    ...
    status["decision_reason"] = "reviewed-preparation 审计 accepted_for_draft 且 reviewed_allowed=true；已生成 formal reviewed knowledge。"
        ...
        log.append(
            {
                "at": TODAY,
                "actor": "codex",
                "action": "phase41_formal_reviewed_created",
...
def mark_needs_more_evidence(candidate: dict[str, Any], decision: dict[str, Any]) -> None:
    ...
    status["decision_reason"] = str(decision.get("reason", "reviewed-preparation 审计仍要求补充证据。"))
    status["updated_at"] = TODAY
    ...
    machine_gate["reason"] = "reviewed-preparation 审计仍为 needs_more_evidence；不能生成 formal reviewed 或默认指导。"
        ...
        log.append(
            {
                "at": TODAY,
                "actor": "codex",
                "action": "phase41_reviewed_preparation_needs_more_evidence",
                "reason": f"{TASK_ID}: 审计仍需补证，保持 candidate，不生成 formal reviewed。",
            }
        )
...
def main() -> int:
    ...
    for decision in audit_result["decisions"]:
        ...
        if decision.get("decision") == "accepted_for_draft" and decision.get("reviewed_allowed") is True:
            ...
            skipped["rejected_not_implemented"] += 1

    if len(promoted) != 20:
        raise ValueError(f"Expected 20 promotions, got {len(promoted)}; skipped={dict(skipped)}")
    if len(needs_more) != 2:
        raise ValueError(f"Expected 2 needs_more_evidence, got {len(needs_more)}")
    ...
    write_json(
        REMAINING_FOLLOWUP_PATH,
        {
            ...
            "generated_at": TODAY,
            "audit_result_id": AUDIT_RESULT_ID,
            "remaining_count": len(needs_more),
            "items": needs_more,
            "boundary": "Remaining items are candidates only; no reviewed/approved/default guidance/hard gate.",
        },
    )
    ...
    report = {
        "report_id": "phase41_reviewed_preparation_import_report",
        "generated_at": TODAY,
        "task_id": TASK_ID,
        "audit_result_id": AUDIT_RESULT_ID,
        "source_package_id": SOURCE_PACKAGE_ID,
