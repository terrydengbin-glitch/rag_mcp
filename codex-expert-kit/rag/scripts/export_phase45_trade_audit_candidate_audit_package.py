"""Export Phase 45 Audit Trail / Clock Sync candidates for strict external audit."""
...
PACKAGE_GATE = resolve_repo_path("docs", "reports", "phase45_trade_audit_candidate_audit_package_quality_gate.json", start_file=__file__)
...
def load_candidates() -> list[dict[str, Any]]:
    ...
    for partition in PARTITIONS:
        ...
        for path in sorted(cand_dir.glob("cand_20260612_phase45_trade_audit_*.json")):
            data = read_json(path)
...
def package_quality_gate(candidates: list[dict[str, Any]]) -> dict[str, Any]:
    failures: list[str] = []
    if len(candidates) != 6:
        failures.append(f"expected 6 candidate_ready Audit Trail candidates, got {len(candidates)}")
    ...
    return {
        "gate_id": "phase45_trade_audit_candidate_audit_package_quality_gate",
        "checked_at": TODAY,
        "phase": PHASE,
        "task_id": TASK_ID,
        ...
        "warnings": [
            ...
            "Audit Trail / Clock Sync 只能用于交易审计链、事件时间、ID 映射、幂等和日志完整性，不得证明策略 alpha。",
            "SEC/CAT/FINRA/ESMA 来源具有辖区边界；FIX/NIST 只能作为协议和日志治理支撑。",
        ],
    }
...
def build_package(candidates: list[dict[str, Any]], gate: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "scope": {
            ...
            "partitions": PARTITIONS,
            "batch": "P45-B Audit Trail / Clock Sync / 审计追踪与时钟同步",
            "candidate_count": len(candidates),
            "target": "审计 Audit Trail / Clock Sync 候选，包括业务时钟同步、订单事件因果链、ID 映射、事件序列/幂等、日志 retention/integrity、人工与电子订单时间戳边界。",
        },
        "hard_boundaries": {
        ...
        "audit_instructions": [
            "必须搜索相关的专业网站、官方文档、监管资料、协议文档、标准、案例和数据，对本审计包进行严格审计。",
            "逐条检查来源是否足以支撑 claim；SEC/CAT/FINRA/ESMA/FIX/NIST 不得被过度泛化。",
            "检查 Audit Trail / Clock Sync 是否正确分区，不得把策略 alpha、回测指标、实盘风控 hard gate 或 AI Engineering 本体混进本分支。",
            "检查每条候选是否清楚声明适用范围、不适用场景、假设、限制、来源质量和 AI 使用边界。",
            "重点审计 clock synchronization、order-event causality、client/exchange order-id mapping、event sequence/idempotency、audit log retention/integrity、manual vs electronic timestamp boundary。",
            ...
            "candidate_results": [
                {
                    ...
                    "classification_assessment": {
                        ...
                        "expected_branch": "Trading Engineering / Audit Trail / Clock Sync",
