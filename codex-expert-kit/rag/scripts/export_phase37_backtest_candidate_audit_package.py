"""Export Phase 37 Backtest candidates for strict external audit."""
...
PACKAGE_GATE = resolve_repo_path(
    "docs", "reports", "phase37_backtest_candidate_audit_package_quality_gate.json", start_file=__file__
)
...
def load_candidates() -> list[dict[str, Any]]:
    candidates = []
    for path in sorted(CAND_DIR.glob("cand_20260611_phase37_backtest_*.json")):
        data = read_json(path)
        if (
...
def package_quality_gate(candidates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    expected_tasks = {f"P37-E-B{idx:02d}" for idx in range(1, 13)}
    ...
    warnings.append("Backtest 知识只能约束回测可信度、偏差、成本、验证和复现边界，不得证明任何策略盈利。")
    return {
        "gate_id": "phase37_backtest_candidate_audit_package_quality_gate",
        "checked_at": TODAY,
        "phase": PHASE,
        "task_id": TASK_ID,
...
def build_package(candidates: list[dict[str, Any]], gate: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "scope": {
            ...
            "batch": "P37-E Backtest / 回测可信度",
            "candidate_count": len(candidates),
            "target": "审计回测可信度本体知识候选，包括 lookahead/data leakage、survivorship、参数搜索、walk-forward、样本外、成本、滑点、指标解释和复现版本。",
        },
        "hard_boundaries": {
        ...
        "audit_instructions": [
            ...
            "逐条检查来源是否足以支撑 claim，教育材料、平台文档和框架文档不得被过度泛化。",
            "检查是否正确归类到 Trading Engineering / Backtest，不得混入 Data Engineering、Replay、Live Execution、Risk Management 或 AI Engineering 本体。",
            "检查每条候选是否清楚声明适用范围、不适用场景、假设、限制、来源质量和 AI 使用边界。",
            "重点审计 lookahead bias、data leakage、data snooping、过拟合、walk-forward、样本外验证、交易成本、滑点、fill 假设、指标解释和可复现性。",
